...
function stubNestedAtaHandlers(app: ElectronApplication) {
  return app.evaluate(({ ipcMain }, txId) => {
...
test.describe('Solana nested ATA recovery', () => {
  ...
  test('shows validation errors when submitted empty', async () => {
    ...
    await expect(page.locator('[name="userKey"]')).toHaveClass(
      /tw-border-red-500/
    );
    await expect(page.locator('[name="nestedAtaAddress"]')).toHaveClass(
      /tw-border-red-500/
    );
  });
  ...
  test('completes recovery and shows txId on success page', async () => {
    ...
    await page.fill(
      '[name="userKey"]',
      '{"iv":"abc","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","ct":"encryptedUserKey"}'
    );
    await page.fill(
      '[name="backupKey"]',
      '{"iv":"def","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","ct":"encryptedBackupKey"}'
    );
    await page.fill(
      '[name="bitgoKey"]',
      'xpub661MyMwAqRbcGHoJePkfLFqgzNBjqAqMjZaRVPMd3su2mTJ7HJQNFVkBkzVHBG7yWVZvLgXSZDmDj8mqqVFnzWijVhKanGj6ygSWfzFQ6eB'
    );
    ...
    await page.waitForURL(/non-bitgo-recovery\/solNestedATA\/success/, {
      timeout: 10_000,
    });
