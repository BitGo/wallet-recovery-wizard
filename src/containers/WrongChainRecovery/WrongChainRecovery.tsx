...
export function WrongChainRecovery() {
  ...
  const [sourceCoin, setSourceCoin] = useState<string>(sourceCoins[0].value);

  const destinationCoins = useMemo(() => {
    return wrongChainRecoveryCoins[environment][sourceCoin];
  }, [environment, sourceCoin]);
  const [destinationCoin, setDestinationCoin] = useState<string>(
