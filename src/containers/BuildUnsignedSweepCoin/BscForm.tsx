...
export type BscLikeFormProps = {
  coinName: string;
  onSubmit: (
    values: BscLikeFormValues,
    formikHelpers: FormikHelpers<BscLikeFormValues>
  ) => void | Promise<void>;
};

type BscLikeFormValues = Yup.Asserts<typeof validationSchema>;

export function BscForm({ onSubmit, coinName }: BscLikeFormProps) {
  const formik = useFormik<BscLikeFormValues>({
    onSubmit,
    initialValues: {
      apiKey: '',
      backupKey: '',
  ...
  console.log('BscForm', allCoinMetas[coinName]);
  ...
  return (
    <FormikProvider value={formik}>
      <Form>
        ...
        <div className="tw-mb-4">
          <FormikTextfield
            HelperText={`Gas limit for the BSC transaction. The value should be between ${
        ...
        <div className="tw-mb-4">
          <FormikTextfield
            HelperText={`An Api-Key Token from bscscan.com required for recoveries.`}
            Label="API Key"
            name="apiKey"
            Width="fill"
          />
